...
and Bootstrap support the user interfaces. Continuous integration and continuous deployment are in place utilizing CircleCI. Code edited locally and pushed to GitHub will initialize the CI/CD workflow, which will automatically test, build and push the application container image to Google's Container Registry (GCR), and deploy the container to Cloud Run. A load test is run afterwards to ensure application functionality.
